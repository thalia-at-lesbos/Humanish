...
4. **Check whether any documents need updating** (`docs/ref/`, `docs/user/` including the in-game encyclopedia, `docs/planning/`, `CLAUDE.md`, `CHANGELOG.md`, `docs/design/`) and update the affected ones per the Documentation tier rules below — only `docs/design/` changes **require the user's explicit consent** first.
...
- `provisional_sections` (game-rules.md and ai-design.md) — list of `⚠️ Provisional` subsections with placeholder constants
...
`StartMenu` loads `DataDB`, shows title + buttons (New Game / Load Game / Multiplayer / Multiplayer Server / About / Exit — the two multiplayer routes are described under Remote multiplayer below), and instantiates `SetupScreen` on "New Game". When setup completes it calls `main.init_with_facade(facade, db)` before adding `main.tscn` to the scene tree, then frees itself. `main.gd` falls back to a default 2-player tiny game when `init_with_facade` is not called (e.g. running `main.tscn` directly).
...
| `CultureLevels` | §15.4 culture levels (pure static, D2+C4): the single reader of the reference geometric border/defence curve — per-pace `culture_level_thresholds` (paces.json) → `level_for` (border ring = level + 1), `defence_pct` (`culture_level_defence`, the culture-level city defence), `legendary_threshold` (cultural victory). Read by `TurnEngine`, `Combat`, `WinConditions`, `CityScreen`. |
...
| `Quests` | Multi-turn quest subsystem (pure static): a `TurnEngine` player-step phase mirroring `Events` that arms eligible quests from `data/quests.json`, tracks aim/constraint progress on `gs.active_quests`, and applies rewards through the §9 event effect verbs (`Events.apply_effects`); multi-choice rewards reuse the event popup path via `gs.pending_event_choices`. |
...
- **New society**: add an entry to the `"societies"` block in `data/leaders_traits.json` with `id`, `name`, `leader_id`, `leader_name`, `description`, `traits` (array of trait IDs), `starting_gold`, `starting_techs` (must yield exactly a settler plus one tech-derived escort unit), and `city_names` (exactly 20 distinct non-empty names, capital first) — `test_data_db.gd` enforces the last two. Optional: `unique_unit`, `unique_building`, `release`. It will appear automatically in the `SetupScreen` society picker.
