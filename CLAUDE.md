...
- **New random event (§9)**: add the event definition to `data/events.json` (name, text, optional `choices[]` each with `effects[]`, or a bare begin `effects[]`; a positive `duration` + `expire_effects[]` makes it a timed event) and a predicate to `data/event_triggers.json` (`event_id` plus any of `min_turn`/`max_turn`/`tech_required`/`building_required`/`terrain_required`/`at_war`/`at_peace`/`probability`/`weight`/`one_shot`). `Events` reads both directly — no code unless you introduce a *new effect verb* (add a `case` to `Events._apply_effect` and the validator's `known` list in `DataDB._validate_event_effects`). Keep effect magnitudes fixed integers so a human-resolved choice stays deterministic.
