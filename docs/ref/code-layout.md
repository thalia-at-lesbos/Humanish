...
| `units.json` | Domain, strength, movement, cost, upkeep, tags, first strikes, combat limit, `bombard_rate` (§15.4 city-defence bombardment points per mission), `ocean_capable` (sea hulls cleared for deep-water crossing) |
| `structures.json` | Settlement building costs, output bonuses, specialist slots (no per-building gold upkeep — dropped for reference parity with C1; the `upkeep` read path remains for mods). Per-structure `science_bonus` is a summed % multiplier on the city's research commerce share (`_apply_research`); effects keys `unhealthy_global` (owner-wide unhealth, Three Gorges Dam), `happiness_with_<resource>` (conditional comfort while the resource is accessible, Hippodrome), `culture_rate_happiness` (§15.13/§29.12, M2 — the entertainment tier: theatre/pavilion 10, hippodrome 20, colosseum/odeon/ball_court/garden 5, broadcast_tower 10; a city's summed carrier values × the owner's `slider_culture` / 100, truncated once per city in `_update_contentment`) and `enemy_war_weariness` (accrual % on enemies, Statue of Zeus) are read by `TurnEngine`/`CombatApply`. A top-level `not_buildable: true` (M7 — Military Academy) bars the structure from every city production queue (`SimFacade._cmd_set_production` rejects it; `PlayerAI._sorted_options` never offers it) while non-queue grants still work (the Great General's `build_military_academy` action, event rewards); `DataDB._validate_structure_not_buildable` requires the flag to be boolean and the structure to keep a grant path (a GP `build_<sid>` action or an event/quest `building` reward). Per-structure `obsoleted_by` (§15.17/§29.15, M1 — 23 shipped entries, e.g. walls→rifling, Hagia Sophia→steam_power, Apostolic Palace→mass_media) names the tech at which the owner's copy stops contributing **everything** (yields, happiness/health, defence, XP, slots, `effects` reads); the building remains built (never sold, no refund) and keeps identity-only uses (wonder score, upkeep, build-uniqueness, name/cost display). The predicate is `Player.structure_obsolete(db, sid)`; every standing-structure aggregation site filters through it (`DataDB._validate_structure_obsolete_refs` checks the tech ids) |
| `technologies.json` | Research cost, prereq graph (`prereqs_all`, `prereqs_any`), unlocks. The `unlocks_units` lists are **display-only** (tech-chooser tooltip, encyclopedia, `TextGen`) and follow one convention (W8): a non-unique unit appears under exactly **one** tech — its *final gating tech*, the entry of its `tech_required` set latest by era then by cost; unique units are omitted (they surface via their society's roster/encyclopedia pages instead). `test_data_db.gd` cross-checks every entry against the unit's `tech_required`. A tech may carry `worker_speed_modifier` (M1: Steam Power +50, §15.9), summed into `TurnEngine.worker_build_turns` over the player's researched techs — Steam Power is also Hagia Sophia's `obsoleted_by` tech, so the wonder's identical +50 hands over without double-stacking |
...
| `promotions.json` | Per-promotion combat bonuses, applies-to filter. `collateral_damage_protection` (Drill II–IV, 20 each, summing) cuts spillover damage taken (`CombatApply.spillover_taken`, W5); `same_tile_heal`/`adjacent_tile_heal` (Medic line, Woodsman III) feed the heal-phase single-best medic bonus (`TurnEngine._medic_bonus`, W6) |
...
| `ages.json` / `paces.json` / `difficulties.json` | Scaling multipliers and per-level modifiers; each pace row also carries the §15.4 `culture_level_thresholds` border/defence curve |
...
| `projects.json` | Projects, two kinds: endgame (spaceship-style) stages — cost, tech/wonder gate, stage and count-needed; feeds the `endgame_project` win condition — and §15.7 **effects projects** (`sdi`, `the_internet`): no `win_condition`, an `instances` limit ("player"/"world"), optional `requires_wonder_any`, and an `effects` dict read via `Projects.effect_int` (`nuke_interception` 75, `tech_share` 2) |
...
1. Resolve/expire trade offers, then deliver every active persistent deal's recurring per-turn items (`_execute_deals`, §7: gold-per-turn transfers; a deal lapses if a party is gone or the two alliances are at war). Diplomatic memory also decays one step here (`Diplomacy.decay`), and so does war weariness for wars that have ended (`_decay_war_fatigue`, §15.8: −`war_weariness_decay_rate` then keep `war_weariness_decay_peace_percent`; no decay while either side still lists the war)
...
- Structure upkeep charged to treasury (`_settlement_upkeep` — a mod hook: shipped structures carry no `upkeep` field since the C1 building-upkeep retune); the same phase heals the city's bombardment `defence_damage` by `city_defence_heal_rate` (5) points (§15.4)

### `Player`
Per-player economic and research state. The four allocation rates (`slider_finance`, `slider_research`, `slider_culture`, `slider_intel`) sum to 100 — the `SET_SLIDERS` command carries only the three adjustable rates (research/culture/intel) and `SimFacade._cmd_set_sliders` derives `slider_finance` as the remainder (100 − the three), so the serialized fields and save format are unchanged. `split_commerce(total)` partitions a settlement's commerce output into `[finance, research, culture, intel]` according to the rates. Also holds: `state_religion` (§8.1 — the player's adopted belief, switching it triggers anarchy via the shared anarchy counter); `era` (§2.1 — a *cache* of the derived current era, recomputed by `Eras.refresh`; every rule reads it live through `Eras.player_era`); `intel_points` (§7.1 — espionage points accumulated per rival alliance, spent on missions); Golden Age state (`golden_age_turns` / `golden_age_count` / `pending_golden_age_gp`); and Great General accumulation (`great_general_points` / `great_general_threshold` / `great_generals_produced`). Also `society_id` (the player's society, set in `SimFacade.setup` from cfg or recovered from `leader_id`) and `used_city_names` (historical city names already assigned, so nameless foundings draw the next unused entry of the society's `city_names` list, capital first). All serialized. `structure_obsolete(db, sid)` is the single §15.17 obsolescence predicate (owner has researched the structure's `obsoleted_by` tech): filtered at `TurnEngine._structure_effect_active` (contentment/production) and at every other standing-structure aggregation loop — yields/granary carry/wellbeing incl. `unhealthy_global`/power gate/unit XP/free promotions/worker speed/`heals_units`/siege HP (`city_max_health`, optional owner arg)/`science_bonus`/espionage flat+output (`TurnEngine`), `Combat.settlement_defence` (optional owner arg), `CombatApply` `enemy_war_weariness`, `Specialists.slots_for`, `GlobalWarming` building unhealth, `Nuclear` (nukes-enabled/shelter/meltdown), `Assembly` founding-wonder gates, `GreatPeople._player_has_active_structure` (Great Wall/Mausoleum), and the facade's whip-anger/espionage-defense/city-intel readouts plus `PlayerAI`'s missionary gate. Identity uses (presence, wonder score, upkeep, build-uniqueness, destroy-building targeting, name lists) deliberately do not filter.

### `Settlement`
Holds all per-city state: population, food store, production queue and store, culture total and border ring (= culture level + 1, §15.4), bombardment damage to the culture defence (`defence_damage`, serialized int), contentment/wellbeing breakdowns, specialist assignments, and a list of built structure IDs. `effective_workers()` = `population − discontented`.

### `Unit`
Per-unit state: position, `base_strength`, `health` (0–100), experience, promotions, movement allowance (fixed-point), entrenchment, and worker state. `effective_strength(db, is_attacker, terrain, feature, versus_class)` computes the final integer strength factoring in all stacked percentage modifiers from promotions, terrain defence, and entrenchment, then scales by `health / 100`. Class-versus-class `vs_<class>` keys (`Unit.VS_CLASS_KEY`) are read from **both** the unit's promotions and its own data row (W7: panzer carries a unit-level `vs_armor: 50`), stacking additively into the one bonus sum.

### `Stack`
Stateless helpers that query `gs.units` by tile position. `at(units, x, y, player_id)` lists units at a tile. `get_defender(...)` returns the highest-strength enemy unit at a tile — skipping `espionage`-tagged units (spies cannot be attacked, §7.1) and `classification: "missile"` units (missiles cannot defend, §15.7/D3; a stranded missile dies instead via `CombatApply.destroy_stranded_missiles`).
...
Also hosts the city-defence sum the resolver and the facade readouts share: `settlement_defence(settle, db)` = per-structure `defence_bonus` + `cultural_defence_bonus` + `culture_defence(settle, db)` — the §15.4 culture-level modifier (`CultureLevels.defence_pct(level)`, level = `culture_ring − 1`), degraded proportionally by the settlement's bombardment `defence_damage` (0..`max_city_defence_damage`).

### `CombatApply`
Pure application of a resolved `Combat.resolve()` result back to `GameState`: healths, XP, auto-promotions, removing the dead, advancing a victorious attacker onto the tile, spillover/flanking damage (each stacked unit's summed promotion `collateral_damage_protection` cuts the spillover it takes — `spillover_taken`, W5/§29.16: damage × (100 − protection)/100 truncating, ≥100 = immunity), war weariness (§15.8 per-event weights — both sides accrue on a unit death; also hosts the shared accruer `accrue_war_fatigue(gs, side_pid, enemy_pid, key)` used by `SimFacade._city_falls` and `Nuclear.detonate`: weight × `war_weariness_multiplier`, forced-war −50%, then ×(100 + the enemy's summed standing-structure `enemy_war_weariness`)/100 — Statue of Zeus +100% — Golden-Age freeze, wild forces skipped), and Great-General accrual. No signals or `Node` references — so both `SimFacade` (the human/AI command path) and `WildAI` (the world-step raider path) share one source of truth. Each caller emits its own signals afterwards from the returned `result`. Also hosts `destroy_stranded_missiles(gs, x, y, captor_pid)` (§15.7/D3): removes hostile `classification: "missile"` units left on a tile with no surviving defender when it is taken — called after a victorious advance, on city capture/raze (`SimFacade`), on a wild raze (`WildAI`), and when a stack walks onto a lone missile's tile (`_cmd_move_stack`).
...
### `Research`
`can_research(tech_id, player, db)` checks `prereqs_all` (all required) and `prereqs_any` (at least one; an empty list imposes nothing) — since D1 the shipped graph is the reference AND/OR tree (92 techs, mostly OR-prereqs). `_effective_cost()` applies pace scaling, a discount per held prereq from either list (10% each), and a discount for each other player who already knows the tech (5% each, capped at 25%).

### `Alliance`
Tracks war state (`at_war_with`), contacts, subordination, shared research store, war fatigue (per-enemy-alliance §15.8 weariness points), `forced_wars` (§15.8: enemy alliances whose war was not this side's choice — declared on it, vassal drag-in, or a nuclear first strike — accruing weariness at the −50% forced-war modifier; cleared at peace; int-coerced on load), and pending trade offers. War and peace are declared at the alliance level, not the player level. Accepted trades carrying recurring items promote to persistent deals on `gs.deals` (§7, see `TurnEngine._execute_deals` / `SimFacade._cmd_cancel_deal`).
...
### Other sim modules
- **`CultureLevels`** — §15.4 culture levels (pure static reader, D2+C4): the single reader of the reference geometric border/defence curve. `thresholds(db, pace_id)` / `level_for(db, pace_id, culture_total)` read the per-pace `culture_level_thresholds` column in `data/paces.json` (5 levels; border ring = level + 1); `defence_pct(db, level)` reads `culture_level_defence` (constants.json — +20/40/60/80/100% city defence at fledgling..legendary, out-of-range levels clamp); `legendary_threshold` is the cultural-victory requirement (`WinConditions._cultural`); `level_name` reads `culture_level_names` for the city screen. Consumed by `TurnEngine._settlement_culture`, `Combat.culture_defence`, `WinConditions` and the `CityScreen`
...
- **`PolicyEffects`** — §8 civic-effects reader (pure static): `sum_int`/`has_flag` aggregate a player's active policies' `effects` (both nested `effects` dicts and bare top-level flags); `largest_city_ids`/`is_religious_structure`/`civic_pressure_anger` (§15.9 emancipation-pressure anger from rivals' civics) are supporting helpers. The single reader of per-civic `effects`, called from `TurnEngine` (happiness/health incl. civic pressure, tile + capital output, production via `_policy_production_delta`, research/intel, treasury, Great-Person rate, new-unit XP, cottage upgrade rate, worker speed via `worker_build_turns`) and `SimFacade` (rush gating). The mechanical policy fields stay in `SimFacade`/`TurnEngine`. See `docs/planning/designgaps.md` §2 for the wired-vs-inert breakdown
- **`TraitEffects`** — §4/B4 leader-trait effects reader (pure static, the `PolicyEffects` analogue for `data/leaders_traits.json` traits): `sum_int` sums a numeric key across a player's traits; `production_pct` returns the summed trait build-speed +% toward a production-queue item — `double_production_structures` lists (+`trait_double_production_pct`, 100, per listing trait: Aggressive barracks/drydock, Protective walls/castle, Organized courthouse/lighthouse, Expansive granary/harbor, Industrious forge, Creative theatre/colosseum) and the per-unit `unit_production_modifiers` dict (Imperialistic settler +50, Expansive worker +25). Wired into `TurnEngine._production_percent_mods` so trait modifiers stack additively with the §4.3 percent chain. Other trait keys are still read in place (`health_bonus` in `TurnEngine`, `promotion_xp_reduction` in `CombatApply`, …)
- **`Projects`** — §15.7 effects-projects reader (pure static, the `PolicyEffects` analogue for `data/projects.json`): a project entry without `win_condition: "endgame_project"` (SDI, The Internet) is recorded on `Player.projects` at completion (`TurnEngine._complete_item`; spaceship stages keep the per-alliance `endgame_project_stages` model) and its `effects` dict is aggregated via `effect_int(player, db, key)` — read by `Nuclear.try_intercept` (`nuke_interception`) and `TurnEngine._apply_tech_share` (`tech_share`, the Internet's absorb-widely-known-techs pass in the PLAYER_RESEARCH phase). `can_build` gates the SET_PRODUCTION queue (tech + `instances: "player"|"world"` limit + `requires_wonder_any`, e.g. SDI's anyone's-Manhattan-Project); `grantable` re-checks the instance limit at completion so a world-unique project finished second grants nothing
- **`Beliefs`** — founding (first-eligible random draw), passive spread within range each turn
- **`EconOrgs`** — §14.6/§15.10 corporations: founded by a special person/Great Merchant (`found` erects the corporation's `hq_structure` in the founding city), organically `spread_all` like beliefs (costs treasury) or deliberately via an executive unit (`SimFacade._cmd_spread_corporation`, the `spread_corporation`-tagged unit, charged `corporation_executive_spread_cost`). Availability is `accessible_resource_counts` (resource → instance count: one per owned connected tile copy, per supplying recurring deal, and per operating corporation with `produces_resource` the player hosts) with `accessible_resources` as its presence-set view (also the §15.12 unit resource gate). Per-city output is `output_per_resource[channel] × accessible input instances / 100` (×1/100 fixed, truncating): food/production fold into the city yield via `get_output_delta`; culture/research/gold route via `settlement_channel` into `_settlement_culture` / `_apply_research` / `gold_income` (outside the commerce split, like specialists). `maintenance_for` charges `maintenance_per_resource × instances / 100` per member city (Free Market `corporation_maintenance_reduction` halves it) and `hq_gold_for` pays the founder `hq_gold_per_franchise` per member city worldwide — both wired into `TurnEngine._update_treasury`. Mercantilism/State Property set the `corporations_disabled` civic flag, under which a player's corporations produce nothing, cost no maintenance, grant no produced resource, and cannot be spread into. Keyed off `data/econ_orgs.json` (reference §29.6 rates); HQ structures carry a `corporation_hq` flag so they are granted by founding, never offered as a normal build
...
- **`Nuclear`** — §5.7 nuclear weapons & radioactive fallout (pure static): `is_nuke(db, unit)` checks the `nuke` tag; `try_intercept` rolls ONE interception chance per strike — the best of an in-range anti-air unit (`nuke_interception_chance`) and any target-side owner's SDI project effect (`nuke_interception`, reference 75) — before `detonate(gs, attacker, tx, ty, rng)` resolves the blast: per-unit damage 30+rand(50)+rand(50) (combat units floored at 1 health; non-combatants killed at the 60 threshold), per-settlement population death 30+rand(20)+rand(20) % plus a 40% destruction roll per structure, ground stripping (improvement/feature removal), and Fallout contamination (50% per blast tile). The meltdown world-tick runs inside `TurnEngine.world_step`. Magnitudes are the §15.7 reference block in `data/constants.json` (`nuke_*`); every stochastic step draws from the shared `gs.rng` in fixed tile order
