...
func _guide_sections() -> Array:
	return [
		...
		["Economy Rates", [
			...
			"minimum, is disabled.",
			"",
			"Entertainment buildings (Theatre, Colosseum, Broadcast Tower and unique",
			"variants) also grant happiness scaling with the Culture rate: a city sums",
			"their values (Theatre +1 happy per 10% culture, Colosseum +1 per 20%),",
			"multiplies by the rate, and rounds down once.",
		]],
		["Cities", [
			"Founding",
			...
			"A citizen with no tile to work and no specialist post automatically works as a",
			"Citizen specialist (+1 production) until land or slots open up again.",
			"Rush Production — spend 3 gold per remaining hammer to instantly finish the",
			"current item (always available, no unhappiness; +50% if queued this turn).",
			"",
			"Growth",
			...
			"",
			"Culture and Borders",
			"Cities produce culture each turn; accumulated culture lifts the city through",
			"six culture levels (Poor to Legendary; at Normal pace 10 / 100 / 500 / 5,000 /",
			"50,000 culture), each pushing its border one ring further out. A tile belongs",
			"to the player with the highest culture there. From Fledgling up the level also",
			"grants the city an intrinsic defence bonus (+20% per level, to +100%); enemy",
			"bombardment can wear it down and it recovers 5 points per turn.",
		]],
		["Units", [
			"Selection",
		...
		["Great People", [
			...
			"(the Military Academy can ONLY be built by a Great General — never from a city queue)",
			"",
			"Every Great Person can also permanently join a city as a super-specialist,",
			"and most types can start a Golden Age. A settled Great Person is FREE — it",
			"consumes no population slot. A settled Great General yields nothing but is a",
			"military instructor: +2 starting XP to every combat unit trained in its city.",
		...
		["Victory Conditions", [
			...
			"",
			"Cultural",
			"Bring three of your cities to Legendary culture (50,000 points each at",
			...
			"Space Race (build the 16 spaceship parts after the Apollo Program;",
			"one of every part type launches the ship for a paced travel countdown) and",
