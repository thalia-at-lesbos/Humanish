...
# shipped slice events are all instant, so timed-machinery tests build their own.
# The event carries its own prereq/active/weight inline (the reworked schema folds
# triggers into the event); active 100 keeps it in every roster. Returns the event
# id. Inject into every DataDB a test touches (e.g. both the original and a reload
# db) so an active instance can resolve its expiry on each.
func register_test_timed_event(db, dur = 5):
	db.events["test_sickness"] = {
		"id": "test_sickness", "name": "Test Sickness", "text": "A test malady.",
		"active": 100, "weight": 30, "prereq": {},
		"duration": dur,
